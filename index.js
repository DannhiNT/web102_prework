...
function filterUnfundedOnly() {
    ...
    const unfunfedGames = GAMES_JSON.filter(game => game.pledged < game.goal);

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(unfunfedGames);
}
...
const numOfUnfundedGame = GAMES_JSON.filter(game => game.pledged < game.goal).length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalPledged.toLocaleString()} has been raised for ${totalGames} games. Currently, ${numOfUnfundedGame} game${numOfUnfundedGame === 1 ? "" : "s"} remain unfunded. We need your help to fund these amazing games!`;
